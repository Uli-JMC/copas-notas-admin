 ...
 * - ✅ FIX 2026-02-18: SI el input existe y viene vacío -> BORRA (guarda "")
 *   (Antes: rescataba el valor viejo y no se podía borrar)
 */

(function () {
  const VERSION = "2026-02-18.1"; // ✅ bump por fix: borrar valores cuando input vacío
  ...
  const saveActiveEvent = withLock(async function () {
    ...
    if (state.schema.selectV2 === true) {
      // ✅ FIX: si el input existe, lo que venga se guarda (vacío = borrar)
      // Si el input NO existe, no tocamos el valor existente.
      if (imgDesk.exists) payload.img_desktop = imgDesk.value;       // "" => borra
      if (imgMob.exists)  payload.img_mobile  = imgMob.value;        // "" => borra
      if (moreImg.exists) payload.more_img    = moreImg.value;       // "" => borra
      if (moreAlt.exists) payload.more_img_alt= moreAlt.value;       // "" => borra

      // ✅ Mantener img como fallback general:
      // Prioridad: img_mobile > img_desktop > evImg (payload.img)
      // (sin rescatar el viejo si vos lo borraste)
      const desk = (payload.img_desktop != null) ? payload.img_desktop : (ev.img_desktop || "");
      const mob  = (payload.img_mobile  != null) ? payload.img_mobile  : (ev.img_mobile  || "");
      const base = payload.img || "";
      const fallbackImg = mob || desk || base;
